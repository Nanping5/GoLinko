...
请勿将敏感配置（如 configs/configs.toml）上传至仓库。请参考 configs/configs.toml.example 填写本地配置，example 文件可安全上传。
...
详细表结构与字段说明见 [docs/model.md](docs/model.md)。
...
- 前端开发建议使用 VSCode，支持热更新和 TypeScript 检查。
- 后端支持热重载开发（可用 air、fresh 等工具），建议本地 MySQL/Redis/Kafka 环境。
- 推荐使用 Postman 进行接口联调，接口集合见 Golink_postman.json。


## 常见问题 FAQ

1. 启动报数据库/Redis/Kafka 连接失败？请检查 configs.toml 配置与服务状态。
2. 前端接口跨域？请确保后端已配置 CORS，或用 Nginx 代理。
3. 静态资源上传失败？请检查 static 目录权限与 .gitignore 设置。
4. 邮箱/短信验证码收不到？请检查 SMTP/短信网关配置。

如有其他问题，欢迎提 Issue 或加入社区讨论。
...
3. 遵循项目代码风格和提交规范
4. 建议配合 example 配置和 .gitignore，避免泄露敏感信息

## 社区与支持

如需帮助、建议或反馈，欢迎提交 Issue 或 PR。
也可通过邮箱、社区群等方式联系维护者。
