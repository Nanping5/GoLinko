...
func (s *sessionService) OpenSession(ctx context.Context, send_id, receive_id string) (string, string, int) {
	...
	// 写操作：重新打开会话时，取消当前用户的隐藏状态
	var writeErr error
	ctx, writeErr = dao.WriteAndMark(ctx, func(db *gorm.DB) error {
		return db.Unscoped().Where("user_id = ? AND session_id = ?", send_id, session.Uuid).Delete(&model.UserSessionHide{}).Error
	})
	if writeErr != nil {
		zlog.GetLogger().Error("清理会话隐藏状态失败", zap.String("user_id", send_id), zap.String("session_id", session.Uuid), zap.Error(writeErr))
	}
...
func (s *sessionService) CreateNewSession(ctx context.Context, req request.CreateNewSessionRequest) (string, string, int) {
	...
	if err != nil {
		errStr := err.Error()
		if errStr == "群聊不存在" || errStr == "群聊已被禁用" || errStr == "用户不存在" || errStr == "用户已被禁用" || errStr == "非法的ID格式" {
			return errStr, "", -2
		}
...
func (s *sessionService) GetSessionList(ctx context.Context, send_id string) (string, []response.GetSessionListResponse, int) {
	...
	if err := myredis.SetKeyEx("session_list_"+send_id, string(respBytes), 60*constants.REDIS_TIMEOUT); err != nil {
...
func (s *sessionService) GetGroupSessionList(ctx context.Context, send_id string) (string, []response.GetGroupSessionListResponse, int) {
	...
	if err := myredis.SetKeyEx("group_session_list_"+send_id, string(respBytes), 60*constants.REDIS_TIMEOUT); err != nil {
		zlog.GetLogger().Error(err.Error())
	}
	return "查询群聊会话列表成功", resp, 0
}
