...
type Client struct {
	...
	stopHeart  chan struct{}    // 停止心跳信号
	writeMutex sync.Mutex       // 保护并发写入
}
...
func (c *Client) Read() {
	...
	for {
		...
		if err != nil {
			...
			// 安全：强制使用 JWT 鉴权后的用户ID覆盖客户端传入的 SendId
			...
			if message.Type == message_type_enum.AudioOrVideo {
				if len(ChatServer.Transmit) < constants.CHANNEL_SIZE {
					...
					c.SendBack <- MessageBack{Message: []byte("服务器繁忙，请稍后再发送")}
				}
				continue
			}
			if messageMode == "kafka" {
				if err := PublishMessageToKafka(message.SessionId, modifiedMessage); err != nil {
					...
					c.SendBack <- MessageBack{Message: []byte("消息发送失败，请稍后重试")}
				}
				continue
			}
...
// StartHeartbeat 启动心跳续期 goroutine（分布式模式）
// 定期刷新用户在线状态，防止 Redis 中的在线状态过期
func (c *Client) StartHeartbeat() {
	ticker := time.NewTicker(myredis.UserHeartbeatInterval * time.Second)
	defer ticker.Stop()
	...
	for {
		...
		case <-ticker.C:
			// 刷新用户在线状态
			if err := myredis.RefreshUserOnline(c.Uuid); err != nil {
				zlog.GetLogger().Error("心跳续期失败", zap.String("uuid", c.Uuid), zap.Error(err))
			}
		}
	}
}

// StopHeartbeat 停止心跳续期
func (c *Client) StopHeartbeat() {
	select {
	case c.stopHeart <- struct{}{}:
	default:
	}
}
