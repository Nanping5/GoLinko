...
func (k *KafkaService) KafkaInit() {
	...
	// 生成实例唯一的消费者组 ID（分布式支持）
	// 多实例部署时，每个实例需要独立的消费者组 ID
	instanceID := os.Getenv("INSTANCE_ID")
	if instanceID == "" {
		instanceID = uuid.New().String()[:8]
	}
	consumerGroupID := fmt.Sprintf("GoLinkoGroup-%s", instanceID)
